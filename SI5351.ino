...
.fine-btn{width:28px;height:28px;border-radius:6px;border:1px solid var(--border);background:rgba(255,255,255,0.06);color:var(--muted);cursor:pointer;font-size:12px;font-weight:600;margin-top:8px;}
.fine-btn.active{background:var(--accent);color:#fff;border-color:var(--accent);}
...
const stepOptions=[1,10,100,1000,10000,100000,1000000];let steps=[1000,1000,1000];let fineTune=[false,false,false];let fineBase=[0,0,0];
...
function updateFineRange(i){const sl=document.getElementById('s'+i);if(!sl)return;const fi=document.getElementById('f'+i);if(fineTune[i]){const base=fineBase[i]||Number(sl.value);const step=steps[i];const maxHz=Math.min(MAX_HZ,base+step*100);sl.min=base;sl.max=maxHz;const cur=Number(sl.value);if(cur<base||cur>maxHz){sl.value=base;if(fi)fi.value=formatDisplay(base,unitModes[i]);}}else{sl.min=MIN_HZ;sl.max=MAX_HZ;}}
function mkChannel(idx,ch){return '<div class="card">'+
  '<div class="card-body">'+
    ...
    '<label>'+t('freq')+': <div class="freq-row"><input type="number" id="f'+idx+'" min="'+(10000/unitMult[unitModes[idx]])+'" max="'+(160000000/unitMult[unitModes[idx]])+'" step="'+(steps[idx]/unitMult[unitModes[idx]])+'" value="'+formatDisplay(ch.freq,unitModes[idx])+'" '+(idx>0&&state&&state.sync_mode?'disabled':'')+'></div></label>'+
  ...
  '<div class="slider-col">'+
    ...
    '<button type="button" class="fine-btn'+(fineTune[idx]?' active':'')+'" id="ft'+idx+'" title="Fine Tune">T</button>'+
  '</div>'+
'</div>'; }
