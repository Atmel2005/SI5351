...
function updateStepUI(i){const rb=document.getElementById('r'+i);const rl=document.getElementById('rl'+i);if(rb)rb.value=stepIndex(steps[i]);if(rl)rl.textContent=t('stepLabel')+' '+formatStep(steps[i]);const fi=document.getElementById('f'+i);const sl=document.getElementById('s'+i);if(fi)fi.step=steps[i];if(sl)sl.step=steps[i];if(sl){const hz=snapHz(Number(sl.value),i);sl.value=hz;if(fi)fi.value=formatDisplay(hz,unitModes[i]);}}
