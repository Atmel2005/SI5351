...
const SLIDER_STEPS=100; // fixed number of slider positions
...
function sliderToHz(pos,i){const step=steps[i]||1;return MIN_HZ+pos*step;}
function hzToSlider(hz,i){const step=steps[i]||1;return Math.round((hz-MIN_HZ)/step);}
...
function updateStepUI(i){const rb=document.getElementById('r'+i);const rl=document.getElementById('rl'+i);if(rb)rb.value=stepIndex(steps[i]);if(rl)rl.textContent=t('stepLabel')+' '+formatStep(steps[i]);const sl=document.getElementById('s'+i);const fi=document.getElementById('f'+i);if(!sl)return;const hz=Number(sl.dataset.hz||sl.value);const pos=Math.min(SLIDER_STEPS,Math.max(0,hzToSlider(hz,i)));sl.value=pos;sl.dataset.hz=sliderToHz(pos,i);if(fi)fi.value=formatDisplay(sliderToHz(pos,i),unitModes[i]);}
function mkChannel(idx,ch){return '<div class="card">'+
  ...
  '<div class="slider-col">'+
    '<input class="freq-slider" type="range" id="s'+idx+'" min="0" max="'+SLIDER_STEPS+'" step="1" value="'+hzToSlider(ch.freq,idx)+'" data-hz="'+ch.freq+'" '+(idx>0&&state&&state.sync_mode?'disabled':'')+'> ' +
...
function updateSyncFields(){const sync=document.getElementById('sync').checked;for(let i=1;i<3;i++){const f=document.getElementById('f'+i);const s=document.getElementById('s'+i);if(f){f.disabled=sync;if(sync){f.value=document.getElementById('f0').value;}}if(s){s.disabled=sync;if(sync){const s0=document.getElementById('s0');const hz=Number(s0.dataset.hz||sliderToHz(Number(s0.value),0));const pos=Math.min(SLIDER_STEPS,Math.max(0,hzToSlider(hz,i)));s.value=pos;s.dataset.hz=sliderToHz(pos,i);}}}}
function attachHandlers(){const syncEl=document.getElementById('sync');const syncBtn=document.getElementById('syncbtn');const langSel=document.getElementById('lang');if(langSel&&!langSel.dataset.bound){langSel.addEventListener('change',()=>{lang=langSel.value||'de';render();});langSel.value=lang;langSel.dataset.bound='1';}if(syncBtn&&!syncBtn.dataset.bound){syncBtn.addEventListener('click',()=>{if(syncEl){syncEl.checked=!syncEl.checked;}syncBtn.classList.toggle('active',syncEl&&syncEl.checked);updateSyncFields();applyLive();});syncBtn.dataset.bound='1';}if(syncEl&&!syncEl.dataset.bound){syncEl.addEventListener('change',()=>{if(syncBtn)syncBtn.classList.toggle('active',syncEl.checked);updateSyncFields();applyLive();});syncEl.dataset.bound='1';}const f0=document.getElementById('f0');if(f0&&!f0.dataset.bound){f0.addEventListener('input',function(){if(document.getElementById('sync').checked){for(let i=1;i<3;i++){const f=document.getElementById('f'+i);const s=document.getElementById('s'+i);const hz=snapHz(Number(this.value)*unitMult[unitModes[i]],i);if(f){f.value=formatDisplay(hz,unitModes[i]);}if(s){const pos=Math.min(SLIDER_STEPS,Math.max(0,hzToSlider(hz,i)));s.value=pos;s.dataset.hz=sliderToHz(pos,i);}}}applyLive();});f0.dataset.bound='1';}for(let i=0;i<3;i++){['d','p','pl','ds','iv','im','e'].forEach(prefix=>{const el=document.getElementById(prefix+ i);if(el&&!el.dataset.bound){el.addEventListener('change',applyLive);el.dataset.bound='1';}});const fi=document.getElementById('f'+i);if(fi&&!fi.dataset.bound){fi.addEventListener('input',function(){const hz=snapHz(Number(this.value)*unitMult[unitModes[i]],i);const s=document.getElementById('s'+i);if(s){const pos=Math.min(SLIDER_STEPS,Math.max(0,hzToSlider(hz,i)));s.value=pos;s.dataset.hz=sliderToHz(pos,i);}this.value=formatDisplay(hz,unitModes[i]);applyLive();});fi.dataset.bound='1';}const sl=document.getElementById('s'+i);if(sl&&!sl.dataset.bound){sl.addEventListener('input',function(){const pos=Number(this.value);const hz=sliderToHz(pos,i);this.dataset.hz=hz;const fi=document.getElementById('f'+i);if(fi)fi.value=formatDisplay(hz,unitModes[i]);applyLive();});sl.dataset.bound='1';const rb=document.getElementById('r'+i);if(rb&&!rb.dataset.bound){rb.addEventListener('input',function(){const pos=Math.max(0,Math.min(6,Number(this.value)||0));steps[i]=stepOptions[pos];updateStepUI(i);});rb.dataset.bound='1';}const ut=document.getElementById('ut'+i);if(ut&&!ut.dataset.bound){ut.addEventListener('click',function(){unitModes[i]=(unitModes[i]+1)%3;ut.textContent=unitLabel(unitModes[i]);const s=document.getElementById('s'+i);const hz=s?Number(s.dataset.hz||sliderToHz(Number(s.value),i)):0;const fi=document.getElementById('f'+i);if(fi)fi.value=formatDisplay(hz,unitModes[i]);});ut.dataset.bound='1';}updateStepUI(i);} }
